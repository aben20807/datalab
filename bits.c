...
int byteSwap(int x, int n, int m)
{
    ...
    int n_mask = (0xff << n);
    int m_mask = (0xff << m);
    int n_byte = (((x & n_mask) >> n) & 0xff) << m;
    int m_byte = (((x & m_mask) >> m) & 0xff) << n;
    x = (x & ~n_mask) | m_byte;
    x = (x & ~m_mask) | n_byte;
    return x;
}
...
{
    int true_mask = ~0 + !x;
    return (true_mask & y) + ((~true_mask) & z);
}
...
{
    // 0xffca3fff = 0b11111111110010100011111111111111
    // 0xd7 << 14 = 0b00000000001101011100000000000000
    return ~(0xd7 << 14);
}
